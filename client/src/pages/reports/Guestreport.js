...
const Guestreport = () => {
    ...
    const handleDownloadReport = (format) => {
        ...
        window.location.href = `/api/guest_reports/export?${params}`;
    };

    const handleGenerateReport = async () => {
        try {
            const response = await axios.get('/api/guest_reports', {
                params: {
                    start_date: startDate,
                    end_date: endDate,
                },
    ...
    return (
        <div className="guest-report-container">
            <h1>Generate Guest Report</h1>
